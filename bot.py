...
class ChatDropCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, helpers: dict):
        ...
        # Pre-parse custom emoji id if provided like <:name:id>
        self._emoji_id = None
        m = re.match(r"^<a?:\w+:(\d+)>$", str(self.CHATDROP_REACTION_EMOJI))
        if m:
            try:
    ...
    def _channel_is_active(self, channel_id: int) -> bool:
        ...
        self._prune_activity(q, now_ts)
        # "active" = >= 4 messages in the window
        return len(q) >= 4
    ...
    async def _create_chat_drop(self, channel: discord.abc.Messageable, channel_id: int):
        ...
        # === Tweaked: resolve and add custom emoji safely ===
        try:
            em_obj = None
            guild = getattr(channel, "guild", None)
            if guild and self._emoji_id:
                em_obj = discord.utils.get(guild.emojis, id=self._emoji_id)
            await msg.add_reaction(em_obj or self.CHATDROP_REACTION_EMOJI)
        except Exception:
            # If custom emoji isn't available, ignore
            pass
    ...
    # ===== Public listeners/commands (non-destructive) =====
    @commands.Cog.listener("on_message")
    async def _chat_activity_listener(self, message: discord.Message):
        # Coexists with your existing on_message. We exit early for bots/DMs.
        if not message.guild or message.author.bot:
            return

        ch_id = message.channel.id
        # Skip if it's a command message starting with "!" so we don't inflate activity
        if message.content and message.content.strip().startswith("!"):
            return

        # Record activity and maybe spawn a drop
        self._activity_bump(ch_id)

        # Soft decay if it's been too long
    ...
    async def _claim_by_reaction(self, payload: discord.RawReactionActionEvent):
        ...
        # === Tweaked: robust reaction matching ===
        if self._emoji_id:
            if not payload.emoji.id or payload.emoji.id != self._emoji_id:
                return
        else:
            if str(payload.emoji) != str(self.CHATDROP_REACTION_EMOJI):
                return
